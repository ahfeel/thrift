...
void t_java_generator::generate_java_struct_tostring(ofstream& out,
  ...
  for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter) {
    ...
    if (field->get_type()->is_base_type() && ((t_base_type*)(field->get_type()))->is_binary()) {
      ...
      indent(out) << "  int __" << field->get_name() << "_size = this." << field->get_name() << ".length > 128 ? 128 : this." << field->get_name() << ".length;" << endl;
      indent(out) << "  for (int i = 0; i < __" << field->get_name() << "_size; i++) {" << endl;
      indent(out) << "    if (i != 0) sb.append(\" \");" << endl;
      indent(out) << "    sb.append(Integer.toHexString(this." << field->get_name() << "[i]).length() > 1 ? Integer.toHexString(this." << field->get_name() << "[i]).toUpperCase() : \"0\" + Integer.toHexString(this." << field->get_name() << "[i]).toUpperCase());" <<endl;
      indent(out) << "  }" << endl;
